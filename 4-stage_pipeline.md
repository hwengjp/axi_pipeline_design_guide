...
パイプライン回路の動作原理の説明を節女hしてサンプルコードを提供します。
...
ValidがLの場合、無効なデータ（バブル）がパイプラインを流れます。ReadyがLからHになったサイクルのT0,T1,T2,T3は、ReadyがLのサイクルと同じデータになります。

```
Clock    : 012345678901234
...
ReadyがLの時、パイプラインは停止（ストール）します。ReadyがLからHになったサイクルのT0,T1,T2,T3は、ReadyがLのサイクルと同じデータになります。
...
2. **パイプラインのストール動作**:
   - 7クロック目（Ready=L）: データが保持される
   - 8クロック目（Ready=H）: T0,T1,T2,T3の値が7クロック目と同じになる
